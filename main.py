...
def main():
    ...
    # args.split = 'val'
    # args.eval = 1
    # args.auto_gpu_config = 0
    # args.load = 'tmp/dump/pretained_model/periodic_1440000.pth'
    # args.use_gtsem = 0
    # args.num_processes = 8
    # args.num_eval_episodes = int(2000 / args.num_processes)
    # args.sim_gpu_id = "1"
    # args.sem_gpu_id = 1
    # args.policy_gpu_id = "cuda:0"
    # args.exp_name = args.exp_name +"_"+ datetime.now().strftime("%d_%m_%H_%M_%S")
    ...
    args.use_gtsem = 0
    args.num_processes = 8
    args.save_periodic = 10000
    args.sim_gpu_id = "0"
    args.sem_gpu_id = 1
    args.policy_gpu_id = "cuda:0"
    args.num_training_frames = 2000000
    args.exp_name = args.exp_name +"_"+ datetime.now().strftime("%d_%m_%H_%M_%S")
    ...
    local_w = int(full_w / args.global_downscaling) # 480
    local_h = int(full_h / args.global_downscaling) # 480
    ...
    target_edge_map = np.zeros((num_scenes, 1, local_w,
                            local_h))
    ...
    grid_downscaling = 2
    # grid_map 240 * 240  torch.Size([bs, 1, 240, 240])
    grid_map = nn.MaxPool2d(grid_downscaling)(local_map[:,1:2,:,:])
    # increase_map torch.Size([bs, 1, 480, 480])
    ...
    def init_map_and_pose():
        full_map.fill_(0.) # torch.Size([bs, 20, 960, 960])
        full_pose.fill_(0.) # torch.Size([bs, 3])
        ...
        for e in range(num_scenes):
            ...
            # lmb (bs, 4) local map在global map上面的左上角和右下角的坐标
            lmb[e] = get_local_map_boundaries((loc_r, loc_c),
                                              (local_w, local_h),
                                              (full_w, full_h))

            planner_pose_inputs[e, 3:] = lmb[e]
            # (bs, 3) local map在global map上面的左上角顶点坐标，这是agent全局坐标向局部坐标转化的桥梁
    ...
    def init_map_and_pose_for_env(e):
        ...
        local_ex_map[e]=np.zeros((local_w,
                            local_h))
        target_edge_map[e]=np.zeros((1, local_w,
                            local_h))
    ...
    def remove_small_points(local_ob_map, image, threshold_point, pose):
        ...
        map_x, map_y = int(pose[0].cpu()), int(pose[1].cpu())
        ...
        goal_pose_map[map_x, map_y] = 1
        # goal_map = skimage.morphology.binary_dilation(
        #     goal_pose_map, selem) != True
        # goal_map = 1 - goal_map
        planner.set_multi_goal(goal_pose_map)

        # image中标签的值是从1开始递增的整数，每个不同的整数值代表一个不同的连续区域。
        img_label, num = measure.label(image, connectivity=2, return_num=True)#输出二值图像中所有的连通域
        props = measure.regionprops(img_label)#输出连通域的属性，包括面积等
        Goal_edge = np.zeros((1, img_label.shape[0], img_label.shape[1]))
        ...
        if dict_cost:
            ...
            for i, (key, value) in enumerate(dict_cost):
                # print(i, key)
                Goal_edge[0][img_label == key + 1] = 1
                # Goal_point (480, 480)中将我们选择出来的值得探索的轮廓的中心都标记为对应的i+1
                Goal_point[int(props[key].centroid[0]), int(props[key].centroid[1])] = i+1 #
                if i == 7:
                    # 这里也就是只取得分最高的前三名的边界区域
                    break
    ...
    # 多出来的4层应该是4个探索边界对应的channel
    ngc = 8 + args.num_sem_categories
    es = 2
    # 输入到模型中的BEV图进一步下采样，大小是240 * 240
    ...
    global_input = torch.zeros(num_scenes, ngc - 3, int(local_w/2), int(local_h/2))
    ...
    # 将4层边界的信息加入到模型的输入中作为输入
    # global_input torch.Size([bs, 4 + 2 + 2 +16, 480, 480])
    global_input[:, 0:1, :, :] = nn.MaxPool2d(args.global_downscaling)(torch.from_numpy(target_edge_map).to(device).float())

    global_input[:, 1:, :, :] = nn.MaxPool2d(args.global_downscaling)(local_map[:, 0:, :, :])
    ...
    # 这里面给这个初始值就很关键了, 因为g_rollouts.obs的大小是21,正式收集标签的时候,是从位置开始的
    # g_rollouts.obs torch.Size([20+1, bs, 2+2+4+16, 240, 240])
    g_rollouts.obs[0].copy_(global_input)
    # g_rollouts.extras torch.Size([20+1, bs, 2 ]) 里面装的值一个是朝向, 另一个是目标物体的编号
    ...
    for step in range((args.num_training_frames) // args.num_processes + 1):
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                # origins中的朝向角度始终是0，所以在local_pose和full_pose中的朝向角度没有发生变化，也不会受到影响
            ...
            for e in range(num_scenes):
                ...
                # local_pose torch.Size([1, 3]) 是当前的t时刻agent的local位姿,以m为单位
            ...
            for e in range(num_scenes):
                global_orientation[e] = int((locs[e, 2] + 180.0) / 5.)
            global_input[:, 0:1, :, :] = nn.MaxPool2d(args.global_downscaling)(
        ...
        for e in range(num_scenes):
            ...
            goal_edge = global_action_selection_list[global_item]

            local_goal_maps[e][int(goal_edge[0]), int(goal_edge[1])] = 1
        ...
        if g_step % args.num_global_steps == args.num_global_steps - 1 and l_step == args.num_local_steps - 1:
            if not args.eval:
                ...
                # g_next_value torch.Size([bs]) 
                # args.use_gae = False; args.gamma = 0.99; args.tau = 0.95
