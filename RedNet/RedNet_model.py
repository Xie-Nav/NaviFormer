...
class RedNetResizeWrapper(nn.Module):
    ...
    def forward(self, rgb, depth, train=False):
        ...
        rgb = rgb.float() / 255
        if self.resize:
            rgb = F.interpolate(rgb, self.pretrained_size, mode='bilinear', align_corners=True)
        ...
        if train:
            ...
            # pred_score = (torch.max(scores, 1)[0]).float() # B x 480 x 640

            # print("torch: ", pred[0][100])
            pred[torch.max(scores, 1)[0] < 0.8] = 1
            # pred_score[torch.max(scores, 1)[0] < 0.8] = 0
            ...
            if self.resize:
                ...
                # pred_score = F.interpolate(pred_score.unsqueeze(1), (og_h, og_w), mode='nearest')
